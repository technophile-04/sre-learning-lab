...
- **`docs/adr/0009-non-linear-lesson-index.md`** — the deck is non-linear: a left journey panel (`JourneyPanel.tsx`) groups cards into chapters and drives `goTo()` on click. Chapter grouping is derived in the panel (CONCEPT / TRY IT / SHIP IT / RECAP open chapters), not authored on cards. Verdict marks (✓ pass, ◐ partial, ◌ miss) come from `deck-store.progress`.
- **`docs/adr/0010-always-available-code-peek.md`** — a right-side sheet (`CodePeekSheet.tsx`) shows the current state of `CrowdFund.sol` on demand. Reuses `CodeBlock` directly so the shiki render, vox dim+hover, and ☞ manicule are the same as inline code panels. Opens via a `peek code` button in the top bar or the `c` keyboard shortcut (guarded against text inputs / CodeMirror focus).
