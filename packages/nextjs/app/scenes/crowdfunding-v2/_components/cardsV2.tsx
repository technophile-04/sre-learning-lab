...
function CardFrame({ card, children }: { card: Card; children: React.ReactNode }) {
  ...
  return (
    <div className={`deck-card deck-card-enter ${meta.gotcha ? "card-gotcha" : ""}`}>
      <div style={{ position: "relative", padding: "30px 38px 36px", minHeight: 420 }}>
...
// explanation and let the panel highlight the region (see focusForCard in DeckV2).
...
export function CardRendererV2({ card, chain }: { card: Card; chain: Chain }) {
  switch (card.type) {
    case "concept":
